...
subtest New => sub {
    is CLASS->new( schema_url => 'foo', attributes => { key => 'value' } ), object {
        call      schema_url => 'foo';
        call_hash attributes => { key => 'value' };
    }, 'Arguments to constructor';

    subtest 'Empty environment' => sub {
        is CLASS->new, object {
            call      schema_url => '';
            call_hash attributes => {};
        }, 'No attributes';
    };
...
subtest 'Merge' => sub {
    ...
    is $foo->merge($bar), object {
        call      schema_url => 'foo';
        call_hash attributes => { a => 1, b => 2, c => 2 };
    }, 'Prefer new but keep schema URL when mismatched';

    is $bar->merge($foo), object {
        call      schema_url => 'bar';
        call_hash attributes => { a => 1, b => 1, c => 2 };
    }, 'Confirm preference';

    is $non->merge($foo), object {
        call      schema_url => 'foo';
        call_hash attributes => { a => 1, b => 1 };
